...
const register = async (req, res, next) => {
  try {
    ...
    const { name, email, password, role, companyName, companySize, industry } = req.body;
    ...
    if (role === 'admin' && (!req.user || req.user.role !== 'admin')) {
      return next(new ApiError(403, 'Not authorized to create admin accounts'));
    }
...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    next(new ApiError(500, 'Could not send reset email. Please try again later.'));
  }
};
