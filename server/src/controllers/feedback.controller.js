...
const getAllFeedback = async (req, res, next) => {
	try {
		...
		if (req.user.role === "client") {
			...
			// Filter by feedback assigned to current user
			queryObj.assignedTo = req.user.id;
		}

		// If assignedTo is specified, keep it (it overrides assignedToMe)
...
const getFeedbackById = async (req, res, next) => {
	try {
		...
		if (
			...
			return next(
				new ApiError(403, "Not authorized to access this feedback")
			);
		}
...
const updateFeedback = async (req, res, next) => {
	try {
		...
		if (
			...
			return next(
				new ApiError(403, "Not authorized to update this feedback")
			);
		}
...
const getFeedbackByStatus = async (req, res, next) => {
	try {
		...
		logger.error(
			`Error getting feedback by status ${req.params.status}:`,
			error
		);
		next(error);
	}
...
const getFeedbackByPriority = async (req, res, next) => {
	try {
		...
		logger.error(
			`Error getting feedback by priority ${req.params.priority}:`,
			error
		);
		next(error);
	}
...
const getFeedbackByCategory = async (req, res, next) => {
	try {
		...
		logger.error(
			`Error getting feedback by category ${req.params.categoryId}:`,
			error
		);
		next(error);
	}
...
const changeFeedbackStatus = async (req, res, next) => {
	try {
		...
		if (req.user.role === "client") {
			...
			if (feedback.submittedBy._id.toString() !== req.user.id) {
				return next(
					new ApiError(
						403,
						"Not authorized to change this feedback status"
					)
				);
			}
			...
			if (!["closed", "reopened"].includes(status)) {
				return next(
					new ApiError(403, "Clients can only close or reopen feedback")
				);
			}
		}
		...
		logger.error(
			`Error changing status of feedback with ID ${req.params.id}:`,
			error
		);
		next(error);
	}
...
const addComment = async (req, res, next) => {
	try {
		...
		logger.error(
			`Error adding comment to feedback with ID ${req.params.id}:`,
			error
		);
...
const getComments = async (req, res, next) => {
	try {
		...
		if (
			...
			return next(
				new ApiError(403, "Not authorized to view these comments")
			);
		}
		...
		logger.error(
			`Error getting comments for feedback with ID ${req.params.id}:`,
			error
		);
		next(error);
	}
	...
	try {
		...
		if (
			...
			return next(
				new ApiError(
					403,
					"Not authorized to add attachments to this feedback"
				)
			);
		}
		...
		logger.error(
			`Error adding attachments to feedback with ID ${req.params.id}:`,
			error
		);
...
const getFeedbackHistory = async (req, res, next) => {
	try {
		...
		if (
			...
			return next(
				new ApiError(403, "Not authorized to view this feedback history")
			);
		}
		...
		logger.error(
			`Error getting history for feedback with ID ${req.params.id}:`,
			error
		);
		next(error);
	}
